...
| 문의·신청 | 전화·이메일 직접 연결 | 폼 없음 — 수집하는 개인정보 없음 |
...
폼을 두지 않고 **전화와 이메일로 직접** 연락받습니다. 서버도, 외부 폼 서비스도
필요 없고, 방문자가 개인정보를 입력할 일도 없습니다.

연락처를 바꾸려면 `src/lib/site.ts` 의 `contact` 만 고치면 사이트 전체
(헤더·연락처 페이지·하단·개인정보처리방침)에 한 번에 반영됩니다.
