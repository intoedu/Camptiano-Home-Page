export const ko = {
  meta: {
    localeName: "한국어",
    switchTo: "English",
    dir: "ltr",
  },

  nav: {
    home: "홈",
    about: "소개·인사말",
    programs: "사역·사업 안내",
    news: "공지·소식",
    gallery: "갤러리",
    visit: "오시는 길",
    apply: "신청·예약",
    faq: "자주 묻는 질문",
    contact: "문의하기",
    support: "후원·헌금",
    menu: "메뉴",
    close: "닫기",
    skipToContent: "본문으로 건너뛰기",
  },

  common: {
    learnMore: "자세히 보기",
    viewAll: "전체 보기",
    donate: "후원하기",
    applyNow: "참석 신청하기",
    contactUs: "문의하기",
    backTo: "목록으로",
    comingSoon: "준비 중입니다",
    required: "필수",
    optional: "선택",
    days: "일",
    hours: "시간",
    minutes: "분",
    photoPending: "사진 준비 중",
    contentPending: "내용 준비 중입니다. 자료가 확보되는 대로 채워집니다.",
  },

  home: {
    heroEyebrow: "1950 – 1953 · 한국전쟁 필리핀 참전용사",
    heroTitle: "70년 전 세워진\n작은 비석 하나",
    heroBody:
      "그 돌 하나에 담겨 있던, 잊혀진 전쟁의 잊혀진 영웅 이야기. 캠프티아노 기념비의 주인공 티아노 아폴로 소위와 한국전에 참전하신 필리핀 용사 여러분께 감사의 마음을 전합니다.",
    heroPrimary: "추모식 참석 신청",
    heroSecondary: "이야기 읽기",

    ceremonyLabel: "다가오는 추모식",
    ceremonyHeading: "제{n}주년 캠프티아노 추모식",
    ceremonyCta: "참석 신청",
    ceremonyPassed: "올해 추모식이 마무리되었습니다. 다음 일정은 곧 안내드립니다.",

    missionEyebrow: "우리가 하는 일",
    missionTitle: "기억은 말이 아니라, 남겨진 것으로 이어집니다",
    missionBody:
      "캠프티아노기념사업회는 세 가지 일을 준비하고 있습니다. 후손이 배움을 이어 가게 하는 일, 스러져 간 이름을 남기는 일, 그리고 가족이 간직해 온 얼굴을 모두가 볼 수 있게 하는 일입니다.",

    storyEyebrow: "티아노 소위 이야기",
    storyTitle: "이름은 남았지만, 이야기는 남지 않았습니다",
    storyBody:
      "한국을 지키기 위해 바다를 건너온 청년이 있었습니다. 그가 머물던 자리에는 그의 이름을 딴 캠프가 세워졌고, 70년이 지나 작은 비석 하나만이 그 자리를 지키고 있습니다. 우리는 그 비석에서 시작해, 함께 싸운 7,420명의 이야기를 되찾으려 합니다.",
    storyCta: "사업회 소개 보기",

    newsEyebrow: "공지·소식",
    newsTitle: "사업회 소식",
    newsEmpty: "첫 소식을 준비하고 있습니다.",

    galleryEyebrow: "갤러리",
    galleryTitle: "기록으로 남은 얼굴들",
    galleryBody:
      "유가족이 간직해 온 사진, 추모식의 순간, 그리고 기념비의 사계절을 모으고 있습니다. 사진을 가지고 계시다면 사업회로 연락 주세요.",

    supportEyebrow: "함께해 주세요",
    supportTitle: "한 사람의 이름을 남기는 데,\n한 사람의 마음이 필요합니다",
    supportBody:
      "후원금은 참전용사 후손 장학기금, 전사자의 벽 건립, 사진 기념관 건립에 사용됩니다. 사용 내역은 공지·소식에 정기적으로 공개합니다.",

  },

  visitors: {
    eyebrow: "함께 기억하는 분들",
    today: "오늘",
    total: "누적",
  },

  about: {
    title: "소개·인사말",
    lead: "캠프티아노기념사업회는 한국전쟁에 참전한 필리핀 용사와 그 가족을 기억하고, 감사의 마음을 다음 세대에 전하기 위해 세워졌습니다.",

    greetingHeading: "인사말",
    greetingBody: [
      "70년 전에 세워진 작은 비석 하나에, 잊혀진 전쟁의 잊혀진 영웅 이야기가 담겨 있었습니다.",
      "캠프티아노 기념비의 주인공 티아노 아폴로 소위와 한국전 참전용사 여러분께 감사의 마음을 전합니다.",
      "이 사이트를 통해 티아노 소위님과 필리핀 참전용사 여러분의 가족들이 한국을 방문하게 되기를 바랍니다. 그리고 필리핀 참전용사들의 역할을 몰랐던 한국과 세계의 다음 세대에게도 한국전쟁의 의미와 그분들의 숭고한 희생을 알리고 감사하는 자리가 되기를 바랍니다.",
    ],
    greetingSignoff: "캠프티아노기념사업회",

    storyHeading: "캠프티아노, 그 이름의 유래",
    storyBody:
      "이 자리에는 사업회가 확인한 티아노 아폴로 소위의 생애와 캠프티아노 기념비의 내력이 들어갑니다. 참전 기록, 전사 경위, 비석 건립 경위 등 확인된 사실만 담을 예정입니다.",

    valuesHeading: "우리가 지키는 것",
    values: [
      {
        title: "정확하게 기억합니다",
        body: "확인된 기록만 싣습니다. 확인되지 않은 이야기는 확인될 때까지 싣지 않습니다.",
      },
      {
        title: "가족을 먼저 생각합니다",
        body: "모든 사업의 첫 자리에는 유가족이 있습니다. 사진 한 장, 이름 한 줄도 가족의 동의 위에서 다룹니다.",
      },
      {
        title: "다음 세대에 전합니다",
        body: "한국과 필리핀, 그리고 세계의 청소년들이 이 이야기를 알 수 있도록 한국어와 영어로 함께 기록합니다.",
      },
    ],

    historyHeading: "연혁",
    historyPending: "사업회 연혁을 정리하고 있습니다.",
    orgHeading: "조직",
    orgPending: "조직 구성을 정리하고 있습니다.",
  },

  programs: {
    title: "사역·사업 안내",
    lead: "캠프티아노기념사업회가 준비하고 진행하는 일들입니다.",
    supportLabel: "이 사업 후원하기",
    goalLabel: "목표",
    raisedLabel: "모금액",
  },

  news: {
    title: "공지·소식",
    lead: "추모식 안내, 사업 진행 상황, 후원금 사용 내역을 이곳에 올립니다.",
    empty: "아직 등록된 글이 없습니다.",
    categories: {
      all: "전체",
      notice: "공지",
      event: "행사",
      report: "사업보고",
      press: "언론보도",
    },
    published: "게시일",
  },

  gallery: {
    title: "갤러리",
    lead: "기념비의 사계절, 추모식의 순간, 그리고 유가족이 간직해 온 사진들.",
    tabs: { photo: "사진", video: "영상" },
    empty: "사진을 모으고 있습니다.",
    videoEmpty: "영상을 준비하고 있습니다.",
    contribute: "사진을 가지고 계신가요?",
    contributeBody:
      "아버지, 할아버지의 사진 한 장이 기념관의 시작이 됩니다. 사업회로 연락 주시면 정중히 모시겠습니다.",
  },

  visit: {
    title: "오시는 길",
    lead: "캠프티아노 기념비와 사업회 사무실 위치를 안내합니다.",
    mapPending: "지도는 정확한 위치가 확정되는 대로 연결됩니다.",
    addressLabel: "주소",
    transportHeading: "교통편",
    transportPending: "교통편 안내를 준비하고 있습니다.",
    etiquetteHeading: "참배 안내",
    etiquette: [
      "기념비는 언제든 자유롭게 참배하실 수 있습니다.",
      "단체 참배나 헌화를 계획하고 계시면 미리 알려 주시면 안내해 드리겠습니다.",
      "비석과 주변 조형물은 손상되지 않도록 조심해 주시기 바랍니다.",
    ],
  },

  apply: {
    title: "신청·예약",
    lead: "추모식 참석, 단체 참배, 교육·강연 요청을 접수합니다.",
    typeLabel: "신청 종류",
    types: {
      ceremony: "추모식 참석",
      group: "단체 참배 예약",
      education: "교육·강연 요청",
      family: "유가족 등록",
    },
    nameLabel: "성함",
    orgLabel: "소속 (단체·기관·학교)",
    emailLabel: "이메일",
    phoneLabel: "연락처",
    countLabel: "참석 인원",
    dateLabel: "희망 날짜",
    messageLabel: "남기실 말씀",
    submit: "신청서 보내기",
    privacy:
      "입력하신 정보는 신청 처리 목적으로만 사용하며, 처리 완료 후 파기합니다.",
    privacyAgree: "개인정보 수집·이용에 동의합니다.",
  },

  support: {
    title: "후원·헌금",
    lead: "여러분의 후원이 장학기금, 전사자의 벽, 사진 기념관으로 이어집니다.",
    bankHeading: "계좌 후원",
    bankName: "은행",
    bankAccount: "계좌번호",
    bankHolder: "예금주",
    copy: "복사",
    copied: "복사되었습니다",
    onlineHeading: "간편 후원",
    onlineBody:
      "카카오페이나 토스로 계좌번호를 입력하지 않고 바로 후원하실 수 있습니다.",
    onlinePending:
      "간편 후원 링크를 준비하고 있습니다. 그동안은 계좌 후원 또는 문의하기를 이용해 주세요.",
    corporateHeading: "기업·기관 후원",
    corporateBody:
      "기업, 관공서, 학교, 교회 등 단체 차원의 후원과 협력을 환영합니다. 추모식 공동 주최, 장학기금 매칭, 사진 기념관 건립 참여 등 함께할 수 있는 방법을 상의드리겠습니다.",
    corporateCta: "협력 문의하기",
    receiptHeading: "기부금 영수증",
    receiptPending:
      "기부금 영수증 발급 가능 여부는 확인 후 안내드리겠습니다.",
    transparencyHeading: "사용 내역 공개",
    transparencyBody:
      "모인 후원금이 어디에 어떻게 쓰였는지 공지·소식 게시판에 정기적으로 올립니다.",
    otherHeading: "돈이 아니어도 좋습니다",
    otherBody:
      "사진 자료 제공, 번역 재능기부, 행사 자원봉사, 유가족 연결 — 함께할 수 있는 방법은 많습니다.",
  },

  faq: {
    title: "자주 묻는 질문",
    lead: "가장 많이 주시는 질문을 모았습니다. 찾으시는 답이 없다면 문의해 주세요.",
  },

  contact: {
    title: "문의하기",
    lead: "궁금한 점, 제안, 취재 요청, 유가족 연결 — 무엇이든 편하게 연락 주세요.",
    formHeading: "문의 남기기",
    infoHeading: "직접 연락",
    nameLabel: "성함",
    emailLabel: "이메일",
    phoneLabel: "연락처",
    subjectLabel: "문의 유형",
    subjects: {
      general: "일반 문의",
      donation: "후원 문의",
      family: "유가족·참전용사 관련",
      press: "취재·언론",
      volunteer: "자원봉사·재능기부",
      photo: "사진·자료 제공",
    },
    messageLabel: "내용",
    submit: "보내기",
    sending: "보내는 중…",
    success: "문의가 접수되었습니다. 빠른 시일 안에 답변드리겠습니다.",
    error: "전송에 실패했습니다. 잠시 후 다시 시도하시거나 이메일로 연락 주세요.",
    privacy:
      "입력하신 정보는 문의 답변 목적으로만 사용하며, 답변 완료 후 파기합니다.",
    privacyAgree: "개인정보 수집·이용에 동의합니다.",
  },

  footer: {
    aboutHeading: "사업회",
    linksHeading: "바로가기",
    contactHeading: "연락처",
    followHeading: "소식 받기",
    rights: "캠프티아노기념사업회",
    privacy: "개인정보처리방침",
    terms: "이용약관",
    builtNote: "감사의 마음으로 만들었습니다.",
  },

  notFound: {
    title: "페이지를 찾을 수 없습니다",
    body: "주소가 바뀌었거나, 아직 준비 중인 페이지일 수 있습니다.",
    cta: "홈으로 돌아가기",
  },
};

/** 사전 구조의 기준. 한국어 사전이 원본이고 다른 언어는 이 형태를 따릅니다. */
export type Dictionary = typeof ko;
