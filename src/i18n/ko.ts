export const ko = {
  meta: {
    localeName: "한국어",
    switchTo: "English",
    dir: "ltr",
  },

  nav: {
    home: "홈",
    about: "소개·인사말",
    programs: "사역·사업 안내",
    news: "공지·소식",
    gallery: "갤러리",
    visit: "오시는 길",
    faq: "자주 묻는 질문",
    contact: "연락처",
    support: "후원·헌금",
    menu: "메뉴",
    close: "닫기",
    skipToContent: "본문으로 건너뛰기",
  },

  common: {
    learnMore: "자세히 보기",
    viewAll: "전체 보기",
    donate: "후원하기",
    contactUs: "연락하기",
    call: "전화 걸기",
    email: "이메일 보내기",
    backTo: "목록으로",
    days: "일",
    hours: "시간",
    minutes: "분",
  },

  home: {
    heroEyebrow: "1950 – 1953 · 한국전쟁 필리핀 참전용사",
    heroTitle: "74년 전 세워진\n작은 비석 하나",
    heroBody:
      "그 돌 하나에 담겨 있던, 잊혀진 전쟁의 잊혀진 영웅 이야기. 캠프티아노 기념비의 주인공 아폴로 B. 티아노 소위와 한국전에 참전하신 필리핀 용사 여러분께 감사의 마음을 전합니다.",
    heroPrimary: "추모식 안내 보기",
    heroSecondary: "이야기 읽기",
    heroScroll: "아래로",

    ceremonyLabel: "다가오는 추모식",
    ceremonyHeading: "제{n}주년 캠프티아노 추모식",
    ceremonyCta: "참석 문의",
    ceremonyPassed: "올해 추모식이 마무리되었습니다. 다음 일정은 곧 안내드립니다.",

    inscriptionEyebrow: "돌에 새겨진 말",
    inscriptionTitle: "74년 동안 이 돌은\n같은 말을 하고 있습니다",
    inscriptionBody:
      "전우들이 손수 세운 돌입니다. 새겨 넣은 글자는 74년이 지난 지금도 그대로 남아, 한 사람의 이름과 그가 쓰러진 자리와 날짜를 알려 줍니다.",
    inscriptionThenLabel: "비석이 세워진 날",
    inscriptionThenNote:
      "필리핀 제19대대전투단이 전우의 이름을 새겨 세웠습니다.",
    inscriptionNowLabel: "제74주년 추모식",
    inscriptionNowNote:
      "74년 뒤 같은 날, 같은 자리에서 그 이름을 다시 부릅니다.",
    inscriptionCta: "비문 전문 보기",

    missionEyebrow: "우리가 하는 일",
    missionTitle: "기억은 말이 아니라, 남겨진 것으로 이어집니다",
    missionBody:
      "캠프티아노기념사업회는 세 가지 일을 준비하고 있습니다. 후손이 배움을 이어 가게 하는 일, 스러져 간 이름을 남기는 일, 그리고 가족이 간직해 온 얼굴을 모두가 볼 수 있게 하는 일입니다.",

    storyEyebrow: "티아노 소위 이야기",
    storyTitle: "이름은 남았지만, 이야기는 남지 않았습니다",
    storyBody:
      "1952년 6월 21일, 필리핀 청년 장교 한 사람이 이어리 고지에서 전사했습니다. 그해 9월 18일, 함께 싸운 제19대대전투단 전우들이 그가 지키던 자리에 돌을 세우고 그의 이름을 붙였습니다 — 캠프 티아노. 74년이 지난 지금도 그 돌은 같은 자리에 서 있습니다.",
    storyCta: "사업회 소개 보기",

    newsEyebrow: "공지·소식",
    newsTitle: "사업회 소식",
    newsEmpty: "첫 소식을 준비하고 있습니다.",

    galleryEyebrow: "사진 기념관",
    galleryTitle: "기록으로 남은 얼굴들",
    galleryBody:
      "이 얼굴들을 A4와 A3 크기로 확대해 상설로 걸 수 있는 기념관을 준비하고 있습니다.",
    galleryCta: "갤러리 보기",

    supportEyebrow: "함께해 주세요",
    supportTitle: "한 사람의 이름을 남기는 데,\n한 사람의 마음이 필요합니다",
    supportBody:
      "후원금은 참전용사 후손 장학기금, 전사자의 벽 건립, 사진 기념관 건립에 사용됩니다. 사용 내역은 공지·소식에 정기적으로 공개합니다.",

  },

  visitors: {
    eyebrow: "함께 기억하는 분들",
    today: "오늘",
    total: "누적",
  },

  about: {
    title: "소개·인사말",
    lead: "캠프티아노기념사업회는 한국전쟁에 참전한 필리핀 용사와 그 가족을 기억하고, 감사의 마음을 다음 세대에 전하기 위해 세워졌습니다.",

    greetingHeading: "인사말",
    greetingBody: [
      "74년 전에 세워진 작은 비석 하나에, 잊혀진 전쟁의 잊혀진 영웅 이야기가 담겨 있었습니다.",
      "캠프티아노 기념비의 주인공 아폴로 B. 티아노 소위와 한국전 참전용사 여러분께 감사의 마음을 전합니다.",
      "이 사이트를 통해 티아노 소위님과 필리핀 참전용사 여러분의 가족들이 한국을 방문하게 되기를 바랍니다. 그리고 필리핀 참전용사들의 역할을 몰랐던 한국과 세계의 다음 세대에게도 한국전쟁의 의미와 그분들의 숭고한 희생을 알리고 감사하는 자리가 되기를 바랍니다.",
    ],
    greetingSignoff: "캠프티아노기념사업회",

    storyHeading: "캠프티아노, 그 이름의 유래",
    storyBody:
      "필리핀 카가얀데오로시에서 온 젊은 장교가 있었습니다. 아폴로 B. 티아노 소위. 1952년 6월 21일, 그는 카르와골 이어리 고지에서 전사했습니다.\n\n석 달 뒤인 9월 18일, 함께 싸운 제19대대전투단 전우들이 그가 지키던 자리에 돌 하나를 세웠습니다. 그리고 그 자리를 그의 이름으로 불렀습니다 — 캠프 티아노.",

    valuesHeading: "우리가 지키는 것",
    values: [
      {
        title: "정확하게 기억합니다",
        body: "확인된 기록만 싣습니다. 확인되지 않은 이야기는 확인될 때까지 싣지 않습니다.",
      },
      {
        title: "가족을 먼저 생각합니다",
        body: "모든 사업의 첫 자리에는 유가족이 있습니다. 사진 한 장, 이름 한 줄도 가족의 동의 위에서 다룹니다.",
      },
      {
        title: "다음 세대에 전합니다",
        body: "한국과 필리핀, 그리고 세계의 청소년들이 이 이야기를 알 수 있도록 한국어와 영어로 함께 기록합니다.",
      },
    ],

    historyHeading: "캠프티아노 연표",
    historySource: "위 날짜는 모두 비석에 새겨진 기록에서 확인한 것입니다.",
    orgHeading: "사업회",
  },

  programs: {
    title: "사역·사업 안내",
    lead: "캠프티아노기념사업회가 준비하고 진행하는 일들입니다.",
    supportLabel: "이 사업 후원하기",
    goalLabel: "목표",
    raisedLabel: "모금액",
  },

  news: {
    title: "공지·소식",
    lead: "추모식 안내, 사업 진행 상황, 후원금 사용 내역을 이곳에 올립니다.",
    empty: "아직 등록된 글이 없습니다.",
    categories: {
      all: "전체",
      notice: "공지",
      event: "행사",
      report: "사업보고",
      press: "언론보도",
      memoriam: "부고",
    },
    published: "게시일",
  },

  gallery: {
    title: "갤러리",
    lead: "기념비의 사계절, 추모식의 순간, 그리고 유가족이 간직해 온 사진들.",
    tabs: { photo: "사진", video: "영상" },
    takedownHeading: "실린 사진을 내려 주셨으면 하실 때",
    takedownBody:
      "사진 속 인물이시거나 그 가족이신 분이 원하지 않으시면, 확인한 뒤 곧바로 내립니다. 이유를 밝히지 않으셔도 됩니다. 전화나 이메일로 알려 주세요.",
  },

  visit: {
    title: "오시는 길",
    lead: "캠프티아노 기념비와 사업회 사무실 위치를 안내합니다.",
    addressLabel: "주소",
    heritageEyebrow: "국가보훈부 지정",
    heritageHeading: "현충시설",
    heritageBody:
      "캠프티아노 기념비는 현충시설로 지정되어 있습니다. 나라를 지키기 위해 싸운 이들의 공훈을 기리는 시설로 국가가 인정하고 살피는 자리라는 뜻입니다.",
    heritageAuthority: "지정 기관",
    heritageNumber: "관리번호",
    heritageDate: "지정일",
    heritageKind: "시설 종류",
    heritageManager: "관리주체",
    etiquetteHeading: "참배 안내",
    etiquette: [
      "기념비는 언제든 자유롭게 참배하실 수 있습니다.",
      "단체 참배나 헌화를 계획하고 계시면 미리 알려 주시면 안내해 드리겠습니다.",
      "비석과 주변 조형물은 손상되지 않도록 조심해 주시기 바랍니다.",
    ],
  },

  support: {
    title: "후원·헌금",
    lead: "여러분의 후원이 장학기금, 전사자의 벽, 사진 기념관으로 이어집니다.",
    bankHeading: "계좌 후원",
    bankName: "은행",
    bankAccount: "계좌번호",
    bankPendingBody:
      "사업회 명의의 후원 계좌를 개설하는 중입니다. 확정되는 대로 이 자리에 바로 올리겠습니다.\n\n지금 후원을 원하시면 전화나 이메일로 연락 주세요. 계좌를 안내해 드리고, 원하시는 사업(장학기금·전사자의 벽·사진 기념관)으로 지정해 드리겠습니다.",
    bankHolder: "예금주",
    copy: "복사",
    copied: "복사되었습니다",
    onlineHeading: "간편 후원",
    onlineBody:
      "카카오페이나 토스로 계좌번호를 입력하지 않고 바로 후원하실 수 있습니다.",
    corporateHeading: "기업·기관 후원",
    corporateBody:
      "기업, 관공서, 학교, 교회 등 단체 차원의 후원과 협력을 환영합니다. 추모식 공동 주최, 장학기금 매칭, 사진 기념관 건립 참여 등 함께할 수 있는 방법을 상의드리겠습니다.",
    corporateCta: "협력 문의하기",
    transparencyHeading: "사용 내역 공개",
    transparencyBody:
      "모인 후원금이 어디에 어떻게 쓰였는지 공지·소식 게시판에 정기적으로 올립니다.",
    otherHeading: "돈이 아니어도 좋습니다",
    otherBody:
      "번역 재능기부, 행사 자원봉사, 유가족 연결 — 함께할 수 있는 방법은 많습니다.",
  },

  faq: {
    title: "자주 묻는 질문",
    lead: "가장 많이 주시는 질문을 모았습니다. 찾으시는 답이 없다면 문의해 주세요.",
  },

  contact: {
    title: "연락처",
    lead: "전화나 이메일로 편하게 연락 주세요. 사람이 직접 받고, 직접 답합니다.",
    infoHeading: "직접 연락",
    phoneLabel: "전화",
    emailLabel: "이메일",

    callHeading: "전화로 연락",
    callBody:
      "가장 빠른 방법입니다. 추모식 참석이나 단체 참배처럼 일정 조율이 필요한 일은 전화가 편합니다.",
    callCta: "전화 걸기",

    mailHeading: "이메일로 연락",
    mailBody:
      "자료를 함께 보내실 때, 또는 영어로 연락하실 때 이용해 주세요. 필리핀에서 보내시는 문의도 환영합니다.",
    mailCta: "이메일 보내기",

    reasonsHeading: "이런 일로 연락 주세요",
    reasons: [
      {
        title: "추모식 참석·단체 참배",
        body: "개인·가족 참석은 신청 없이 오셔도 됩니다. 학교, 기관, 기업 등 단체로 오시거나 헌화를 계획하고 계시면 미리 알려 주세요.",
      },
      {
        title: "유가족·참전용사",
        body: "필리핀 참전용사의 가족이시라면 꼭 연락 주세요. 한국어와 영어 모두 가능합니다.",
      },
      {
        title: "후원·협력",
        body: "개인 후원, 기업·기관 협력, 자원봉사와 재능기부 모두 환영합니다.",
      },
      {
        title: "취재·강연 요청",
        body: "언론 취재, 학교와 기관의 교육 프로그램 요청을 받고 있습니다.",
      },
    ],

    noteHeading: "연락하실 때",
    note: "전화를 받지 못하는 경우가 있습니다. 문자나 이메일을 남겨 주시면 반드시 답을 드리겠습니다.",
  },

  footer: {
    aboutHeading: "사업회",
    linksHeading: "바로가기",
    contactHeading: "연락처",
    followHeading: "소식 받기",
    rights: "캠프티아노기념사업회",
    privacy: "개인정보처리방침",
    terms: "이용약관",
    builtNote: "감사의 마음으로 만들었습니다.",
  },

  notFound: {
    title: "페이지를 찾을 수 없습니다",
    body: "주소가 바뀌었거나, 아직 준비 중인 페이지일 수 있습니다.",
    cta: "홈으로 돌아가기",
  },
};

/** 사전 구조의 기준. 한국어 사전이 원본이고 다른 언어는 이 형태를 따릅니다. */
export type Dictionary = typeof ko;
